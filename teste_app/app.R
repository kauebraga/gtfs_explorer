...
server <- function(input, output) {
  ...
  output$map <- renderLeaflet({
    ...
    leaflet() %>%
      addGlPolylines(data = subset(gtfs$shapes, route_type == 1)) %>%
      addGlPolylines(data = subset(gtfs$shapes, route_type == 2)) %>%
      addGlPolylines(data = subset(gtfs$shapes, route_type == 3))
    # 
    # mapdeck() %>%
    #   add_path(data = shapes %>% filter(route_type == 1), layer_id = "id1") %>%
    #   add_path(data = shapes %>% filter(route_type == 2), layer_id = "id2") %>%
